...

{
		...
		this.addWindowListener(new WindowAdapter(){
			public void windowClosing(WindowEvent e){
				...
				try {
				    ...
				    casino.acabado(Integer.parseInt(lblsaldo.getText()), usuario);
				} catch (Exception ex) {
	...
	public void run(){
        ...
        cartas = new Image[40];
        fondo = Toolkit.getDefaultToolkit().getImage(System.getProperty("user.dir")+"/src/juegos/Escoba/Mazo-Escoba/FONDO.GIF");
        mt.addImage(fondo,0);
        ...
        for (j = 0; j<4; j++) {              
            ...
            {  if (j == 0)
                   ...
                   p= "ESPADA";
               if(i == 8)
                   cartas[cont] = Toolkit.getDefaultToolkit().getImage(System.getProperty("user.dir")+"/src/juegos/Escoba/Mazo-Escoba/10"+p+".GIF");  
               else if(i == 9)
                   cartas[cont] = Toolkit.getDefaultToolkit().getImage(System.getProperty("user.dir")+"/src/juegos/Escoba/Mazo-Escoba/11"+p+".GIF");  
               else if(i == 10)
                   cartas[cont] = Toolkit.getDefaultToolkit().getImage(System.getProperty("user.dir")+"/src/juegos/Escoba/Mazo-Escoba/12"+p+".GIF");  
               else
                    cartas[cont] = Toolkit.getDefaultToolkit().getImage(System.getProperty("user.dir")+"/src/juegos/Escoba/Mazo-Escoba/"+Integer.toString(i)+p+".GIF");  
	...
	public void ponerSaldo(int s, UsuarioActivado usuario) {
